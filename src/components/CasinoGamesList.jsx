...
function CasinoGamesList() {
  ...
  console.log("object", allGamesListView?.data?.casino?.length)
  return (
    ...
    <div>
      ...
      <div className="flex items-center justify-between gap-2 w-full  pr-2 mt-4">
        <div className="flex items-center gap-2">
          ...
          <div className="text-[16px] font-extrabold">Casino</div>
        </div>
        {allGamesListView?.data?.casino?.length > 6 ? <div className="flex justify-between">
          <div className="flex items-center">
            <div className="h-5 w-16 rounded-lg border-[0.5px] flex items-center justify-center text-[12px] p-2"> ALL
              <div className="text-[12px] text-customlightbtn pl-1 ">{allGamesListView?.data?.casino?.length}</div>
