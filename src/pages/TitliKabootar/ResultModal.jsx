...
export default function ResultModal({ onClose, announcementData,gameList }) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      {winLoss === 1 ? (
        <div
          className="rounded-lg border-[#c6a120] bg-[#260217] border-[1px] p-3 w-96 h-72 shadow-xl text-white font-serif"
        >
          ...
          <div className="relative flex items-center justify-between text-xsm w-full px-5">
            <img className="absolute top-24 left-[42%] w-14 h-14" src={gameList[announcementData?.data?.number-1]} alt="sdf" />
          </div>
        </div>
      ) : (
        <div
