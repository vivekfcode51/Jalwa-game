...
function K3BetModal({ gameHistoryData, profileDetails, myHistory, bet_api, onClose, gameDetails }) {
    ...
    // console.log("gameHistoryData", gameDetails)
    useEffect(() => {
        ...
        if (checkAgreement) {
            ...
            try {
                ...
                if (res?.data?.status === 200) {
                    ...
                    localStorage.setItem(`betStatus${gameDetails?.gameId}`, 1)
                    // setIsBetDone(res)
                    profileDetails()
                    myHistory()
                    onClose()
    ...
    //     // onClose();
    // };
    const remainder = gameDetails?.betButtonId % 2 === 1
    // console.log("colorClass", colorClass)
    return (
        <>
            {/* <div
                className="fixed h-full inset-0 z-40 flex justify-center items-end"
                onClick={handleOverlayClick}
            > */}
                <div onClick={(e) => e.stopPropagation()} className="w-full xsm:w-[400px] pt-5 bg-white text-black rounded-t-3xl">
                    {/* <div className={`${gameDetails?.colorCode === "rv" ? `bg-[#9B48DB]` : gameDetails?.colorCode === "gv" ? `bg-[#9B48DB]` : "bg-inputBg text-blackLight"} h-[6.6rem] rounded-t-3xl`}>
                        <div className="relative flex flex-col items-center justify-center rounded-t-3xl py-3 text-white" style={{ backgroundColor: colorClass }}>
                            ...
                            <h1 className="text-lg font-bold">Total: {gameDetails?.gameId === 6 ? "1 Minute" : gameDetails?.gameId === 7 ? "3 Minutes" : gameDetails?.gameId === 8 ? "5 Minutes" : "10 Minutes"} </h1>
                    ...
                    <div className='px-4'>
                        {
                            gameDetails?.betType === 20 ?
                                <div className='text-black'>20</div> :
                                gameDetails?.betType === 30 ?
                                    <div>30</div> :
                                    gameDetails?.betType === 40 ?
                                        <div>40</div> :
                                        <div>
                                            <p className="text-black ">Total:</p>
                                            {![40, 50, 60, 70].includes(gameDetails?.betButtonId) ? (
                                                <p className={`rounded-full flex items-center justify-center text-sm w-7 h-7 text-white bg-${remainder ? "customred" : "green"} font-bold`}>
                                                    {gameDetails?.betButtonId}
                                                </p>
                                            ) :
                                                (
                                                    <p style={{ background: colorClass }} className={`rounded items-center justify-center text-sm px-2 flex w-14 h-7 text-white font-bold`}>
                                                        {gameDetails?.betButtonId}
                                                    </p>
                                                )
                                            }
                    ...
                    <div className="w-full mt-5 flex justify-between">
                        ...
                        <div className="flex gap-1 mt-3 px-4 text-white">
                        ...
                        {['X1', 'X5', 'X10', 'X20', 'X50', 'X100'].map((label, i) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`flex items-center justify-center text-xsm rounded-md h-7 px-2 ${quantityIndex === numericValue ? 'text-white' : 'bg-inputBg text-blackLight'
                                        }`}
